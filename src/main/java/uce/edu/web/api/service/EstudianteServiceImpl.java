...
public class EstudianteServiceImpl implements IEstudianteService {
    ...
    private Function<Estudiante, EstudianteTo> mapTo = e -> {
        EstudianteTo eTo = new EstudianteTo(e.getId(), e.getNombre(), e.getApellido(), e.getCedula(), e.getCorreo());
        return eTo;
    };

    private Function<EstudianteTo, Estudiante> mapEstudiante = eTo -> {
        Estudiante e = new Estudiante(eTo.getId(), eTo.getNombre(), eTo.getApellido(), eTo.getCedula(),
                eTo.getCorreo());
        return e;
    };
